...
function pickInfinitive(raw: string): string | null {
  const cleaned = cleanupVerbLabel(raw)
    .replace(/^sich\s+/i, "")
    .trim();
  if (!cleaned) return null;

  const tokens = cleaned.split(/\s+/);
  const infinitiveToken = tokens.find((token) =>
    /(?:en|ern|eln)$/i.test(token),
  );
...
function splitPrefixInfinitive(infinitive: string): {
  prefix: string;
  base: string;
} {
  const prefixes = [
...
function regularParticiple(infinitive: string): string {
  ...
  if (inseparablePrefixes.some((prefix) => lower.startsWith(prefix)))
    return `${stem}t`;
...
export function conjugateVerb(rawVerb: string): VerbDetails | null {
  ...
  if (directPresent) {
    ...
    const praeteritum =
      BASE_IRREGULAR_PRAETERITUM[infinitive] ??
      regularPraeteritumAll(infinitive);
    const futurI = PERSONS.reduce(
      (acc, person) => {
    ...
    return {
      ...
      praeteritumIch:
        praeteritum.ich ??
        directPraeteritumIch ??
        regularPraeteritumIch(infinitive),
  ...
  const baseParticiple =
    BASE_IRREGULAR_PARTICIPLE[base] ?? regularParticiple(base);
  const participle = prefix ? `${prefix}${baseParticiple}` : baseParticiple;
  const basePraeteritum =
    BASE_IRREGULAR_PRAETERITUM[base] ?? regularPraeteritumAll(base);
  const praeteritum = PERSONS.reduce(
    (acc, person) => {
                      ...
                      return (
                        <span className="verb-tooltip" role="tooltip">
                          ...
                          <div className="verb-tooltip-extra">
                            <div>
                              <strong>Perfekt (ich):</strong>{" "}
                              {conjugation.perfectIch}
                            </div>
                            <div>
                              <strong>Praeteritum (ich):</strong>{" "}
                              {conjugation.praeteritumIch}
                            </div>
                            <div>
                              <strong>Konjunktiv II (ich):</strong>{" "}
                              {conjugation.konjunktivIIIch}
                            </div>
                          </div>
                        </span>
                      );
